...
const EditProblemPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const errorMessage = err.response?.data?.message || 'Failed to update problem.';
    ...
    <div className="container mx-auto p-4 text-gray-900 dark:text-gray-200">
      ...
      {problem && (
        <form onSubmit={handleSubmit} className="space-y-6 bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
          ...
          <button type="submit" className="px-6 py-2 bg-green-600 text-white rounded-md hover:bg-green-700">Update Problem</button>
        </form>
      )}
    </div>
  );
